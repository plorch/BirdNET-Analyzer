...
```{r renamefiles}
# Get the function for renameing loaded.
source("C:/Users/PatrickLorch/OneDrive - SSRS/ARU_bird_callanalysis/BirdNET-Analyzer/RenameFiles.R")

# Single dir of .wav files
i.audio = "D:/TNC AudioMoth/THG25_problem"
...
# Single dir of .wav files
i.audio = "D:/TNC AudioMoth/THG25_problem"
o.results = "D:/TNC AudioMoth/Output/THG25_problem"
# audio.folders = paste0(file.path(indir, 
#                            "AudioMoth"), 
#                            29:30)
# result.folders = paste0(file.path(indir,
#                             "results/AudioMoth"),
#                             29:30)
# species.list.path = "D:/species_list.txt"
num.threads = 7 # out of 8 on HP main laptop, 4 on others 
con_limit = 0.1
sens = 1
...
# Delete old results folders first

# This is how the NSNSDA folks worked through multiple folders.
#   We would only want to do this if we can run on a fast computer or over
#   long periods
# Use if you swap comments for the single and multiple folder set up
...
# Since filepath contains the soundfile source, we will use that as RecID
# Will try to create an equivalent verify file using Matt Toenies' methods.
```
...
num.threads = 4 # computer dependent (cores/2 is safe)
...
3. use the warblR commands below
...
You can add records for incidental species detections either in the data_editor or later in the .csv file.
...
# First option makes a wide table that data_edit is not good at managing 
# but works well if you are exporting to an excel sheet
highconf_verified = data.frame(allcolumns, 
                               confirmed = "no",
                               flyover = "no",
                               migrant_spp = "no")
...
# After exiting the editor, the new data should be included in highconf
names(highconf_verified)
write.csv(highconf_verified, file.path(o.results, "highconf_verified.csv"), row.names = F)
...
May be able to use warbleR::read_wave to read in multiple segments and then plot them.
...
source("C:/Users/PatrickLorch/OneDrive - SSRS/ARU_bird_callanalysis/BirdNET-Analyzer/spec_listen.R")

spec_listen(df = sel_table, sel_row = 2580)
